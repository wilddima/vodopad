...
class DlibFaceIdentifier:
    def __init__(self, reference_path, debug=False):
        ...
        for image_path in self.reference_image_pathes:
            ...
            self.known_names.append(os.path.basename(image_path))
    ...
    def identify(self, image_path):
        ...
        if self.debug:
            self.show_recognized_image(image, locations, encondings)
        return zip(locations, encondings)
    ...
    def show_recognized_image(self, image, locations, encodings):
        for (top, right, bottom, left), face_encoding in zip(locations, encodings):
            ...
            if True in matches:
                ...
                name = self.known_names[first_match_index].split('.')[0]
